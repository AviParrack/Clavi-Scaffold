---
name: email-triage
description: "Triage the user's email inbox: classify, surface urgent ones via Telegram, propose reply descriptions, draft replies on approval. Use when the user says 'email triage', 'check email', 'process inbox', 'triage emails', or '/email-triage'."
...
You are running the email triage pipeline for the user's Gmail. **Same shape as `/triage`**: you propose, the user decides. Never write a Gmail draft without the user's explicit sign-off.

## Pattern

Email triage produces a queue file that sits in `Harbor/Inbox/email-triage-{date}.md` until the user engages with it — exactly like how research items pile up in `Harbor/Inbox/` until `/triage` is run. the user reviews when ready, approves which proposed replies to actually draft, Claude writes those Gmail drafts. Urgent + time-sensitive items get a Telegram nudge immediately so the user knows to look.
...
| Tier | Meaning | Action |
|---|---|---|
| **🔴 URGENT** | Time-sensitive AND important. Deadline within 48h, or person waiting on the user to unblock something | Telegram alert immediately + add to queue |
| **🟠 ACTION** | Needs a reply or action from the user, but not urgent | Add to queue with proposed reply |
...
**Calibration on URGENT:** false positives are worse than false negatives. Only flag URGENT when (a) there's a real deadline within ~48h, OR (b) someone is actively blocked waiting on the user. "Urgent" subject-line keywords from senders are not sufficient on their own — judge on actual content. When uncertain, downgrade to ACTION.
...
- "Forward to admin-org.example for handling"
...
**Do not write the actual reply yet.** The description is what the user reads to decide whether to approve. Drafts come later.
...
When the user approves drafts via `--draft <numbers>` arg, or by telling you in chat which numbers to draft, OR via re-invoking `/email-triage --draft 1,3,5`:

1. **Read `Harbor/Dispatch/agents/playbook-email.md` in full before drafting.** This is the authoritative voice and disclosure spec for any email Claude sends on the user's behalf. Every draft is signed by Claude (acting as the user's assistant), opens with an identity disclosure, and closes with a calibration note inviting feedback.
...
**Do not send.** the user sends manually after reviewing.

## Phase 7: Marking processed

Once the user has reviewed and acted on the queue file, update its frontmatter:
...
This step is manual — the user tells you "process the email queue" or "archive that triage" when ready.

## Voice

All email voice rules — persona (Claude as the user's assistant), disclosure (identity open + calibration close), tone, anti-patterns, and the canonical example — live in `Harbor/Dispatch/agents/playbook-email.md`. Read that file in Phase 6 before drafting. Do not invent a voice; defer to the playbook.
...
- **Threads where the user was the last sender** — usually means he's waiting on a reply, not the other way around. Surface as FYI ("waiting on response from {recipient}") unless a reminder is appropriate.
...
If the user corrects a classification ("that one wasn't urgent" or "you missed that this needs action"), note it briefly in `Harbor/Dispatch/scout-calibration.md` so future runs improve. Same shape as scout calibration.
